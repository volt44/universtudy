...
class MongoDbProfilerStorageTest extends AbstractProfilerStorageTest
{
    ...
    public static function setUpBeforeClass()
    {
        if (extension_loaded('mongo')) {
            self::$storage = new DummyMongoDbProfilerStorage('mongodb://127.0.0.1/symfony_tests/profiler_data', '', '', 86400);
            try {
    ...
    {
        return array(
            array('mongodb://127.0.0.1/symfony_tests/profiler_data', array(
                'mongodb://127.0.0.1/symfony_tests',
                'symfony_tests',
                'profiler_data',
            )),
            array('mongodb://user:password@127.0.0.1/symfony_tests/profiler_data', array(
                'mongodb://user:password@127.0.0.1/symfony_tests',
                'symfony_tests',
                'profiler_data',
            )),
            array('mongodb://user:password@127.0.0.1/admin/symfony_tests/profiler_data', array(
                'mongodb://user:password@127.0.0.1/admin',
                'symfony_tests',
                'profiler_data',
            )),
            array('mongodb://user:password@127.0.0.1:27009,127.0.0.1:27010/?replicaSet=rs-name&authSource=admin/symfony_tests/profiler_data', array(
                'mongodb://user:password@127.0.0.1:27009,127.0.0.1:27010/?replicaSet=rs-name&authSource=admin',
                'symfony_tests',
                'profiler_data',
            )),
        );
    ...
    protected function setUp()
    {
        if (self::$storage) {
            ...
            $this->markTestSkipped('MongoDbProfilerStorageTest requires the mongo PHP extension and a MongoDB server on 127.0.0.1');
        }
    }
}
